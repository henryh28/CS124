...
/** Input comment once I know
*/

Phone_Directory::Directory_Entry::Directory_Entry(const std::string& name, const std::string& number)
{
    this->name = name;
    this->number = number;
}
...
void Phone_Directory::Directory_Entry::set_number(string new_number)
{
...
string Phone_Directory::remove_entry(const string& name)
{
    ...
    if (index != -1)
        {
        string erased = the_directory[index].get_number();
